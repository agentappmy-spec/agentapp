...
function App() {
  ...
  useEffect(() => {
    localStorage.setItem('agent_contacts', JSON.stringify(contacts));
  }, [contacts]);
  ...
  const [userProfile, setUserProfile] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : {
      name: 'Dzulfaqar Hashim',
      role: 'super_admin', // Options: 'free', 'pro', 'super_admin'
      planId: 'pro',       // For 'free' users -> 'free', for others -> 'pro'. Super Admin has full access.
  ...
  const openAddModal = () => {
    // --- GATING CHECK: Contacts Limit ---
    // If Super Admin, bypass. Else check plan.
